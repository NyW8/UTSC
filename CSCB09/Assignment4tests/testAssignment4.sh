...
if [ ! $val1 -eq "" ]; then echo Failed test 1: There were errors calling with lorem.txt;
...
if [ ! $val2 -eq "" ]; then echo Failed test 2: There were errors calling with gibberish.txt; 
...
if [ ! $val3 -eq "" ]; then echo Failed test 3: There were errors calling with gibberish-large.txt; 
else echo Passed test 3; fi;

echo; echo TEST 4: SHOULD REFUSE THE CONNECTION:
